...
def run_test_rom_ram(path_rom: str, expected_frames_count: int) -> int:
    ...
    Args:
        ...
        expected_frames_count (int): The exact number of frame the emulation should
            run for until the test succeed.
    """
    nes = FrameCountedNES(path_rom, timeout=expected_frames_count+1)
    ...
    assert nes.frame_count == expected_frames_count, (
        f"Expected the test to succeed in exactly {expected_frames_count} frames, it "
...
def run_test_rom_ppu(
    ...
    Args:
        ...
        failure_matcher (Matcher): Expected string to be printed on the screen in case
            of failure.
        path_rom (str): Path to the test ROM.
        expected_frame_count (int): The exact number of frame the emulation should
            run for until the test succeed.
